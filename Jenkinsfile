      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk saadex/nodeapp-dev:latest'
            sh 'docker push saadex/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk saadex/nodeapp-prod:latest'
                            sh 'docker push saadex/nodeapp-prod:latest'
                            sh 'docker save saadex/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
            }
            post {
                failure {
                    sh 'docker stop nodeapp-dev test-image || true'
...
// Doing containers clean-up to avoid conflicts in future builds
    stage('CLEAN-UP') {
      steps {
        sh 'docker stop nodeapp-dev test-image || true'
        sh 'docker rm nodeapp-dev test-image || true'
        sh 'docker system prune -f'
        deleteDir()
      }
    }
